...
d3.json("./data/relation.json",function(error,root){//读入 JSON 文件
    ...
    var force = d3.layout.force()//力学图的 Layout（布局
        .nodes(root.nodes)
        .links(root.edges)
        .size([width,height])
        .linkDistance(400)
    ...
    var edges_line = svg.selectAll("line")//绘制节点之间的之间
        .data(root.edges)
        .enter()
        .append("line")
        .style("stroke","#555")
    ...
    var edges_text = svg.selectAll(".linetext")//绘制节点之间的直线上的文字
        .data(root.edges)
        .enter()
        .append("text")
        .attr("class","linetext")
    ...
    var nodes_img = svg.selectAll("image")//绘制图片
        .data(root.nodes)
        .enter()
        .append("image")
        .attr("width",img_w)
        ...
        //当鼠标移到图片上时，显示与此结点想关联的连接线上的文字。使用改变透明度的方式。
