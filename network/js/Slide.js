//滑动条程序
network.Slider = network.Class.create();
network.Slider.prototype = {
    //容器对象，滑块
    initialize: function(container, bar, options) {
        this.Bar = network.$id(bar);
        this.Container = network.$id(container);
        this._timer = null;//自动滑移的定时器
        this._ondrag = false;//解决ie的click问题
        //是否最小值、最大值、中间值
        this._IsMin = this._IsMax = this._IsMid = false;
        //实例化一个拖放对象，并限定范围
        this._drag = new network.Drag(this.Bar, { Limit: true, mxContainer: this.Container,
            onStart: network.Bind(this, this.DragStart), onStop: network.Bind(this, this.DragStop), onMove: network.Bind(this, this.Move)
        });
        ...
        //点击控制
        network.addEventHandler(this.Container, "click", network.BindAsEventListener(this, function(e){ this._ondrag || this.ClickCtrl(e);}));
        //取消冒泡，防止跟Container的click冲突
        network.addEventHandler(this.Bar, "click", network.BindAsEventListener(this, function(e){ e.stopPropagation(); }));
        ...
        //修正获取焦点
        var oFocus = network.isIE ? (this.KeyBind(this.Bar), this.Bar) : this.Container;
        network.addEventHandler(this.Bar, "mousedown", function(){ oFocus.focus(); });
    ...
    DragStop: function() {
        this.onDragStop();
        setTimeout(network.Bind(this, function(){ this._ondrag = false; }), 10);
    },
    //滑动中
    Move: function() {
        this.onMove();
    ...
    WheelBind: function(o) {
        //鼠标滚轮控制
        network.addEventHandler(o, network.isIE ? "mousewheel" : "DOMMouseScroll", network.BindAsEventListener(this, this.WheelCtrl));
    },
    //方向键控制
    KeyCtrl: function(e) {
        if(this.KeySpeed){
    ...
    //绑定方向键
    KeyBind: function(o) {
        network.addEventHandler(o, "keydown", network.BindAsEventListener(this, this.KeyCtrl));
        //设置tabIndex使设置对象能支持focus
        o.tabIndex = -1;
        //取消focus时出现的虚线框
        network.isIE || (o.style.outline = "none");
    },
    //获取当前值
    GetValue: function() {
        //根据最大最小值和滑动百分比取值
    ...
    Run: function(bIncrease) {
        ...
        if(!(bIncrease ? this._IsMax : this._IsMin)){
            this._timer = setTimeout(network.Bind(this, this.Run, bIncrease), this.RunTime);
        }
    },
    //停止滑移
    Stop: function() {
    ...
    EasePos: function(iLeftT, iTopT) {
        ...
        if(this._horizontal ? iLeftS : iTopS){
            ...
            //如果没有到极限值则继续缓动
            if(this._IsMid){ this._timer = setTimeout(network.Bind(this, this.EasePos, iLeftT, iTopT), this.RunTime); }
        }
    },
    //获取步长
